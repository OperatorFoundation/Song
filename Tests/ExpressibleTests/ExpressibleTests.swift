...
//        let patternInitializer = PatternInitializer(pattern: pattern, initializerExpression: initializerExpression)
    ...
    func testNWEndpointHost()
    {
        let value = NWEndpoint.Host("127.0.0.1")
        let hostString = "let value = NWEndpoint.Host(\"127.0.0.1\")"
        let source = SourceFile(content: hostString)
        let parser = Parser(source: source)
        ...
        print("\ntopLevelDecl.statements: \(topLevelDecl.statements)\n")
    }
    
    func testNWEndpointPort()
    {
        let value = NWEndpoint.Port(rawValue: 1234)
        let portString = "let value = NWEndpoint.Port(rawValue: 1234)"
        let source = SourceFile(content: portString)
        let parser = Parser(source: source)
